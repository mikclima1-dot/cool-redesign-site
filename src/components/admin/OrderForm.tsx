...
import { money, ORDER_STATUSES, SERVICE_TYPES, type Order, type OrderItem } from "@/lib/admin";
...
export function OrderForm({ order }: { order?: Order }) {
  ...
  const initialItems: OrderItem[] = useMemo(() => {
    const list = Array.isArray(order?.items) ? (order?.items as OrderItem[]) : [];
    if (list.length > 0) return list.map((it) => ({ ...it }));
    if (order && (order.model || order.brand)) {
      return [
        {
          slug: null,
          title: order.model ?? "",
  ...
  const [form, setForm] = useState({
    ...
    service_type: order?.service_type ?? (SERVICE_TYPES[0] as string),
    installation_date: order?.installation_date ?? "",
    installation_time: order?.installation_time ? String(order.installation_time).slice(0, 5) : "",
    notes: order?.notes ?? "",
    total_price: order ? String(order.total_price ?? "") : "",
    paid_amount: order ? String(order.paid_amount ?? "") : "",
    status: order?.status ?? (ORDER_STATUSES[0] as string),
  });
  ...
  const effectiveTotal =
    priceTouched || form.total_price ? Number(form.total_price || 0) : itemsTotal;
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
      <h1 className="text-2xl font-semibold text-brand-navy">
        {isEdit ? `Редакция на поръчка №${order?.order_no}` : "Нова поръчка"}
      </h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {saving ? "Запазване..." : isEdit ? "Запази промените" : "Запази поръчката"}
        </button>
        <button
          type="button"
          onClick={() => navigate({ to: "/admin/porachki" })}
